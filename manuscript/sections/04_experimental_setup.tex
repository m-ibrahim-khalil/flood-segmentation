...
For each $(\text{model} \times \text{KD configuration})$, we report mean $\pm$ standard deviation across the 3 folds. The paired Wilcoxon signed-rank test is applied to the no-KD vs.~comb-KD contrast on each architecture, using fold-level paired IoU as the unit of observation; with $n=3$ folds its minimum reportable two-sided p-value is 0.25, so it functions as a coarse consistency check rather than a powered hypothesis test. Because a three-fold design cannot support conventional significance claims, we characterise the KD and teacher-vs-student contrasts by the \emph{sign} and the \emph{cross-fold consistency} of the mean IoU difference (Section~\ref{sec:results-kd}) and explicitly refrain from powered significance statements. A more powerful per-image paired bootstrap --- pooling per-image IoU over the held-out validation images of all three folds and resampling images with replacement to obtain interval estimates --- is implemented in \texttt{scripts/stats\_bootstrap.py}; it requires a local FSSD copy and was not executed for this version, so no bootstrap confidence intervals are reported here (see Section~\ref{sec:limitations}).

\subsection{Per-Platform Latency Rigging}
\label{sec:setup-latency}

\begin{itemize}
\item \textbf{Apple M2~Pro / ONNX Runtime 1.17 CPU EP:} \texttt{scripts/bench\_m2.py}. Single-threaded by default; sessions created with the intra-op and inter-op thread counts both set to 1. 20 warm-up $+$ 200 timed runs.
