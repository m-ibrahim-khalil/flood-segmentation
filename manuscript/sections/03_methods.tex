...
\item \textbf{Quantization scheme.} Static QDQ (Quantize--DeQuantize) format with \texttt{per\_channel=False} for activations and \texttt{per\_channel=True} for weights restricted to \texttt{op\_types\_to\_quantize=[``Conv'',``Gemm'',``MatMul'']}. The op-type restriction is critical for MobileViT-XXS: without it, the per-channel quantizer attempts to fit per-output-channel scales to the rank-1 weight tensors of LayerNorm and \texttt{Add} operators, which produces undefined behaviour and silently collapses the output.
