...
Beyond the mean comparison, the \emph{direction} of the effect is consistent at the fold level. For the best student, MobileViT-XXS, the combined-KD IoU is below the no-KD IoU in every one of the three folds, and the no-KD MobileViT-XXS exceeds the teacher in every fold (fold-wise margins $+0.013$, $+0.016$, $+0.004$ IoU; mean $+0.011$). Response-only KD likewise trails its no-KD baseline in mean IoU on MobileNetV3-Small ($-0.0040$) and MobileViT-XXS ($-0.0041$). We stress that these are directionally consistent effects on a three-fold design, \emph{not} conventionally-powered significance results: the fold-level Wilcoxon test cannot resolve differences of this magnitude at $n=3$ (minimum $p=0.25$), and the powered per-image bootstrap that would sharpen them was not run for this version (Section~\ref{sec:setup-stats}; a powered re-analysis is the top statistical follow-up, Section~\ref{sec:limitations}). The claim we make is therefore the conservative one: on FSSD, knowledge distillation yields no mean-IoU improvement over a strong ImageNet-pretrained no-KD baseline, and the benchmark is saturated enough that the best student needs no teacher at all.
