...
We still do not recommend the teacher (UNet$+$EffB0) for deployment, but for a subtler reason than raw FP32 speed. On a common ONNX Runtime CPU backend the teacher runs at 37.6\,ms p50 (\teacherMTwoFPS~FPS), only $1.0$--$1.7\times$ slower than the FP32 students, so the case against it rests on \emph{model size and quantization headroom} rather than FP32 latency: the recommended EfficientNet-Lite0 INT8 is $2.2\times$ faster (17.3 vs 37.6\,ms) and roughly $4\times$ smaller (5.2 vs 22.3\,MB) while recovering 95\% of the teacher's IoU, and the teacher does not INT8-quantize within our budget.
