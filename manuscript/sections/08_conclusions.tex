...
% common ORT-CPU backend the teacher is 37.6 ms (cross-checked in lat_m2_ort117.json;
% 36.7 ms when re-timed under ORT 1.22.1 -- version-insensitive), so the honest
% INT8-vs-teacher speedup is 2.2x. Numbers below are final; the M2 CPU rows are
% ORT 1.22.1 (see runs/3fold/lat_m2.json _provenance), the browser rows ORT-Web 1.17.3.
...
We release our trained checkpoints, the ONNX FP32 and INT8 exports, all benchmarking harnesses, and a Zenodo-archived snapshot of the code repository under permissive licences. Future work includes domain adaptation for cross-sensor transfer --- our zero-shot Sen1Floods11 evaluation collapses to IoU $\approx 0.16$ (Section~\ref{sec:results-ood}), so the recommendations hold only for in-domain close-range RGB --- ARM CPU benchmarks on Raspberry~Pi~4 and AWS Graviton, energy measurements, cost-weighted (recall-favouring) evaluation matched to the operational error asymmetry (Section~\ref{sec:discussion-asymmetry}), and quantization-aware-training of the HardSwish and attention-based backbones whose post-training quantization stability we have characterised as a current practitioner trap.
