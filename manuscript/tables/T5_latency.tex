...
\begin{table*}[ht]
\centering
\caption{\textbf{Per-platform inference latency.} Single-image batches, $256{\times}256$ input. M2~Pro and browser WASM both consume the same \texttt{.onnx} artifact under different execution providers. INT8 WASM rows are n/a because ONNX Runtime Web 1.17 does not implement QDQ-format INT8 Conv (upstream limitation; scheduled for ORT-Web 1.18$+$). \textsuperscript{a}Teacher timed under the same ONNX Runtime CPU execution provider as the students (37.6\,ms p50). Under PyTorch eager CPU the same teacher is 274\,ms ($7.3\times$ slower); reporting that PyTorch figure against ORT-CPU students would inflate the apparent teacher-vs-student speedup. On a common ORT-CPU backend the FP32 students are only $1.1$--$1.9\times$ faster than the teacher, and the recommended EfficientNet-Lite0 INT8 is $2.2\times$ faster. All timing rows trace to \texttt{lat\_m2.json} (student rows via \texttt{bench\_m2.py}; teacher ORT-CPU row via \texttt{bench\_teacher\_ort.py}); see \texttt{runs/3fold/PROVENANCE.md}.}
