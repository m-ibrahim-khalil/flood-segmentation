...
**Provenance note (pre-submission integrity pass).** The per-image paired-bootstrap confidence intervals and p-values reported in Sections 3.5/5.2/7 are produced by `scripts/stats_bootstrap.py`, run against a local FSSD copy and the 42 per-fold model checkpoints; the output is committed as `code/runs/3fold/stats_bootstrap.json` and was independently re-run to reproduce the reported values bit-for-bit (seed-fixed). The separate `bootstrap_fold0` entry in `code/runs/3fold/stats.json`, marked "skipped", refers to an earlier fold-0 pixel-bootstrap in `scripts/stats.py` that was superseded by this powered per-image bootstrap and is not used in the manuscript. All numerical values in the manuscript therefore trace to committed artifacts under `code/runs/`.
