...
**Correction (pre-submission integrity pass).** An earlier draft reported per-image paired-bootstrap confidence intervals and p-values for the knowledge-distillation and teacher-vs-student contrasts. That bootstrap (`scripts/stats_bootstrap.py`) was not executed against a local dataset for this version — `code/runs/3fold/stats.json` records it as skipped — so those interval estimates did not trace to any committed result artifact. They have been removed, and the corresponding claims downgraded to fold-level directional statements backed by `code/runs/3fold/{summary,stats}.json`. This note is recorded so that the statement above ("not used to generate numerical values that do not appear in `code/runs/`") is accurate as of submission.
